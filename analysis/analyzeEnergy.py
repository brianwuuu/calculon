...
import util
import collections
from energy_analysis import computeTotalPower, Network
...
sip_power_stats = computeTotalPower(ng=4096,
                                        nl=12288,
                                        ns=768,
                                        link_bw_gbps=400,
                                        switch_radix=16
...
def analyzeModelSizes():
    ...
    sys_params = [
                ...
                # (500,3072,10000,128,"PCIe"), 
                (500,3072,10000,900,"NVLink"), (500,3072,10000,6100,"SiP")
    ...
    job_stats_reformat = []
    for nvlink_t, sip_t in zip(job_stats["NVLink"], job_stats["SiP"]):
        energy_saved = (nvlink_t/1e3*superpod_power_stats["nv"] - sip_t/1e3*sip_power_stats["sip"]) * 2.77778e-10 * 93750
        job_stats_reformat.append(energy_saved)
        
    x_ = {"label": "Model Sizes", "data": model_sizes, "log": None, "limit": None}
    y_ = {"label": "Energy Saved (MWh)", "data": job_stats_reformat, "log": 10, "limit": None}
    # plot_util.plotMultiColBarChart(x = x_, y = y_, path = "")
    fig_path = "/Users/bwu/Downloads/model_sizes.png"
    plot_util.plotBarChart(x = x_, y = y_, path = fig_path)
    
def analyzeNetworkSizes():
    # Models
    model_params = [
            #   (24576,192,128,"1T"), (32768,205,160,"2T"), (40960,213,192,"4T"), (50176,224,224,"7T"),
            #   (60416,236,256,"11T"), (70656,245,288,"18T"), (81920,256,320,"26T"), (94208,268,352,"37T"),
    ...
    model_filename = "24576h_98304ff_8ss_128ah_192as_128nb"
    # Architectures
    arch_params = [
                # (4096,8,32,16)
