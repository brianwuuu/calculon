...
def analyzePicBW():
    ...
    total_length_mm = 96
    per_pic_length_mm = 8
    per_pic_bws_GBps = [200] # 2048, 337.5
    mem_add_lats_ns = [1e0, 1e1, 1e2, 1e3, 1e4, 1e5, 1e6, 1e7, 1e8, 1e9]
    net_lat_ns = 20
    ...
    for workload in workloads.values():
        for mem in mems:
            for per_pic_bw_GBps in per_pic_bws_GBps:
                norm_time = None
                for mem_add_lat_ns in mem_add_lats_ns:
    ...
    x_ = {"label": "Added Mem Latency (s)", "data": [x/1e9 for x in mem_add_lats_ns], "log":10, "limit": None}
    y_ = {"label": "Norm. Iteration Time", "data": job_stats, "log": 10, "limit": (20, 850000)}
    plot_util.plotMultiLineChart(x=x_, y=y_, fig_size=(2,2), bbox_to_anchor=(0,1), ncol=3)
