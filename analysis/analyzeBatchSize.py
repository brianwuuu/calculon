...
def analyzeIterTime():
    ...
    for workload, datatype, per_pic_bw_GBps, mem_add_lat_ns, net_lat_ns, seq_len, max_batch_size in util.cartesian_product(
        ...
        for sys in compute_sys:
            ...
            job_stats[f"{sys_map[sys[0]]}-SiPAM"].append(1/(sipam_time/sipam_time))
            ...
            job_stats[f"{sys_map[sys[0]]}-Baseline"].append(1/(baseline_time/sipam_time))
    ...
    # plot_util.plotMultiColBarChart(x=x_, y=y_, fig_size=(2.3,1.3), bbox_to_anchor=(0.49,0.75), ncol=1)
    plot_util.plotMultiLineChart(x=x_, y=y_, fig_size=(2.3,1.3), bbox_to_anchor=(0.25,1), ncol=3)
...
def analyzeEfficiency():
    ...
    for workload, datatype, per_pic_bw_GBps, mem_add_lat_ns, net_lat_ns, seq_len, max_batch_size in util.cartesian_product(
        ...
        for sys in compute_sys:
            ...
            diff.append(sipam_eff / baseline_eff)
            # job_stats[f"{sys_map[sys[0]]}-Speedup"].append(baseline_time / sipam_time)
...
def analyzeGPUHour():
    ...
    pprint.pprint(job_stats)
    x_ = {"label": "Batch Size", "data": max_batch_sizes, "log":2, "limit": None}
    y_ = {"label": "Total GPU Hours", "data": job_stats, "log":None, "limit": None}
    plot_util.plotMultiLineChart(x=x_, y=y_, fig_size=(1.8,1.5), bbox_to_anchor=(0.25,1), ncol=3)
