...
def optimize_mem_net(gpu, workload, mem, datatype, **kwargs):
    ...
    mem_add_lat_ns = kwargs["mem_add_lat_ns"]
    net_lat_ns = kwargs["net_lat_ns"]
    ...
    # ensures there's always some mem vs net distribution
    max_num_mem_pic_per_gpu = total_length_mm // per_pic_length_mm - 1
    min_num_mem_pic_per_gpu = 1
    
    # new
    # mem_cap_per_gpu_GB = 5 * mem_info['cap_GB']
    # num_gpu = int(np.ceil(workload_info[worktype]["size_GB"] / mem_cap_per_gpu_GB))
    ...
    per_gpu_mem_cap_GB = num_req_mu_per_gpu * mem_info["cap_GB"]
    per_gpu_mem_bw_GBps = num_req_mu_per_gpu * mem_info["bw_GBps"]
    num_gpu = int(np.ceil(workload_info[worktype]["size_GB"] / per_gpu_mem_cap_GB))
    num_gpu = utilities.nearest_pow_of_2(num_gpu)
    par_params = utilities.get_par_params(num_gpu)
    # par_params = (0,0,0)
    ...
    params = dict(
              ...
              workload_size_GB=workload_info[worktype]["size_GB"],
    ...
    mem_params = [{"mem1_GB": per_gpu_mem_cap_GB, 
                   ...
                   "mem1_ns": mem_info["lat_ns"]+mem_add_lat_ns, #30*2, 
                   "mem2_GB": 15000, 
                   "mem2_GBps": per_gpu_mem_bw_GBps, 
                   "mem2_ns": mem_info["lat_ns"]+mem_add_lat_ns, #30*2
                   }]
