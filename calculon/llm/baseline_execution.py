...
class BaselineExecution(calculon.CommandLine):
  NAME = 'llm-baseline-execution'
  ALIASES = ['lbe']

  @staticmethod
  def create_parser(subparser):
    sp = subparser.add_parser(
      BaselineExecution.NAME, aliases=BaselineExecution.ALIASES,
      help='run a simple baseline execution')
    sp.set_defaults(func=BaselineExecution.run_command)
  ...
  def find_min_num_procs_archive(est_num_procs, max_num_procs, app, syst, max_batch_size, worktype, datatype):
    ...
    while power_min <= power_max:
      ...
      n_curr = 2 ** mid_power
      params = BaselineExecution.build_params(n_curr, app, syst, max_batch_size, worktype, datatype)
      output = BaselineExecution.check_capacity(params)
      if output:
  ...
  def check_capacity(params):
    ...
    with mp.Pool(psutil.cpu_count(logical=False)) as pool:
      searches = pool.starmap(BaselineExecution.search, params)
    end_time = datetime.datetime.now()

    # Combines parallel search result into one data structure
    output = BaselineExecution.process_results(searches)
    return output
    ...
    for tp in Llm.get_all_tensor_parallelisms(num_procs, app.hidden, app.attn_heads):
      for pp in Llm.get_all_pipeline_parallelisms(num_procs, tp, app.num_blocks):
        ...
        for ppint in Llm.get_valid_pipeline_interleavings(app.num_blocks, pp):
          batch_size = BaselineExecution.get_batch_size(dp, max_batch_size)
                        ...
                        for dn in pick(dp>1, range(num_nets), [0]):
                          ...
                          if not debug:
                            try:
                              ...
                              best = BaselineExecution.update_list(best, curr, top_n)
  ...
  def process_results(searches):
    ...
    for cbest, ec, gec, bec, tp, pp, dp in searches:
      best = BaselineExecution.update_list(best, cbest, 1)
      exe_count += ec
      good_exe_count += gec
      bad_exe_count += bec
